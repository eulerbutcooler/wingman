...
const QuizTaker: React.FC<QuizTakerProps> = ({ quiz, userId, onComplete, onBack }) => {
      ...
      <div className="bg-white rounded-4xl shadow-sm mb-14 p-8 mx-auto w-full">
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className={`px-3 py-1 rounded-full text-xs text-white ${getDifficultyColor(quiz.difficulty)}`}>
              {quiz.difficulty}
            </span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
            <div className={`h-2.5 rounded-full transition-all duration-300 ${getDifficultyColor(quiz.difficulty)}`} style={{ width: `${progressPercentage}%` }}></div>
          </div>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <button 
            ...
            className={`px-6 py-2 text-white font-bold rounded-4xl cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed transition-colors ${getDifficultyColor(quiz.difficulty)}`}
          >

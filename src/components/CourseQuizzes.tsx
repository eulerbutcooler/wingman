...
const QuizCard: React.FC<QuizCardProps> = ({ quiz, onStart, onReset, onReview }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-6 bg-white p-4 shadow-sm hover:shadow-xl transition-all duration-300 rounded-4xl">
      <div className={`w-full md:w-1/3 text-navy p-6 rounded-4xl flex flex-col justify-center text-center h-full }`}>
        <h2 className="text-xl font-bold">{quiz.difficulty.charAt(0).toUpperCase() + quiz.difficulty.slice(1)}</h2>
      ...
      <div className="w-full md:w-2/3 flex flex-col justify-center">
        ...
        <div className="w-full bg-gray-200 rounded-full h-4 mb-4">
          <div className={`h-4 rounded-full transition-all duration-500 ${getDifficultyBg(quiz.difficulty)}`} style={{ width: `${scorePercentage}%` }}></div>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <button 
            onClick={onStart} 
            className={`flex-grow text-white font-semibold py-2 px-5 rounded-4xl cursor-pointer transition-colors ${getDifficultyBg(quiz.difficulty)}`}
          >
...
const CourseQuizzes: React.FC<CourseQuizzesProps> = ({ courseId, userId, onBackToList }) => {
  ...
  const [currentView, setCurrentView] = useState<'list' | 'quiz' | 'results' | 'review'>('list');
    ...
    <div>
      ...
      {quizzes.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-4xl shadow-sm">
          ...
          <p className="text-sm text-gray-500">Quizzes are being generated for this course.</p>
        </div>
      ) : (

...
// POST /api/chat/rag - RAG-powered chatbot
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { message, courseId, maxSources = 5 } = body;
...
Please provide a detailed answer based on the course materials provided in the context. If you reference specific information, please cite the source number.`;
    ...
    const sources = relevantChunks.map((chunk, index) => ({
      ...
      preview: chunk.chunkText.substring(0, 200) + (chunk.chunkText.length > 200 ? '...' : ''),
    ...
    return NextResponse.json({
      ...
      metadata: {
        ...
        courseStats: {
          ...
          avgChunkTokens: Math.round(parseFloat(courseStats.avg_chunk_tokens as string) || 0)
        }
      }
    });
  ...
  try {
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        avgChunkTokens: Math.round(parseFloat(stats.avg_chunk_tokens as string) || 0),
